...
class FruitsDataStore {
    static let fruitsInstance = FruitsDataStore()
    
    var fruits: [Fruits] = []
    init() {
        self.parseJSONSignFruits()
    }
    
       func parseJSONSignFruits() {
               ...
               do {
                   ...
                   if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String:Any] {
                       ...
                       (json["results"] as? [[String:Any]])?.forEach { j in
                           if let name = j["name"] as? String, let id = j["id"] as? Int {


                               let fruit = Fruits(name: name, number: id)
                               fruits.append(fruit)
                           }
                       }
